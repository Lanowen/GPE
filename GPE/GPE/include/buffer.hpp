...
template<typename T> class buffer
{
  ...
  */
  inline void resize(size_t new_size)
  {
   ...
   //T* new_first = (T*)std::realloc(mFirst,new_size);

	T* new_first = (T*)std::malloc(sizeof(T*) * new_size);
...
class safe_memory
{
 ...
 public:
  ...
  inline void resize(size_t length)
  {
   ...
   Byte* new_memory = (Byte*)std::malloc(length);
   ...
   if (mMemory && mSize)
   {
	...
	std::copy(mMemory, mMemory + copySize, new_memory);
